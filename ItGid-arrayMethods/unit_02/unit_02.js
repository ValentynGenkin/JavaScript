...
const f1 = () => {
  const element = +document.querySelector('.i-1').value;
  document.querySelector('.out-1').textContent = a1.includes(element);
...
const f2 = () => {
  const element = +document.querySelector('.i-2').value;
  a2.includes(element) === true ? document.querySelector('.out-2').textContent = a2.indexOf(element) : document.querySelector('.out-2').textContent = false;
}
...
const f3 = () => {
  const element = +document.querySelector('.i-3').value;
  document.querySelector('.out-3').textContent = a3.flat().includes(element)
}
...
// Выводит в out-4  false, если такого элемента нет и ключ вложенного массива в котором такой элемент есть(если есть). Обратите внимание! Только числа! Если ключей несколько, то выводятся через пробел.
...
const f4 = () => {
  const res = [];
  const element = +document.querySelector('.i-4').value;
  for (let obj in a4){
    a4[obj].includes(element) === true ? res.push(obj) : res.push('False');
}
  document.querySelector('.out-4').textContent = res.join(' ');
}

// TASK 05
// По нажатию b-5 выполняется функция f5. Функция считывает значение из i-5-1 и индекс с которого начинается поиск в массиве с i-5-2 и с помощью includes  ищет данный элемент в массиве a5 c позиции указанной в i-5-2. 
...
document.querySelector('.out-5').textContent = a5.includes(element, index);
}
...
const f6 = () => {
  const element = document.querySelector('.i-6').value;
  document.querySelector('.out-6').textContent = a6.includes(element)
}

// TASK 07
// По нажатию b-7 выполняется функция f7. Функция принимает 2 параметра, первый массив, второй - искомое число. Функция должна эмулировать работу метода includes с помощью цикла. 
...
const f7 = (arr, elem) => {
  for (let item of arr){
   if (elem === item) {
    return document.querySelector('.out-7').textContent = true;
   }
  }
  return document.querySelector('.out-7').textContent = false;
}
...
const f8 = () => {
  ...
  document.querySelector('.out-8').textContent = a8.includes(element)
}

// TASK 09
// По нажатию b-9 выполняется функция f9. Функция должна получить из i-9 символ и с помощью includes искать его в массиве a9. Обратите внимание на сложность. 
// Если пользователь ввел символ в нижнем регистре то функция должна c помощью includes искать и в нижнем и в верхнем регистре символ в массиве. 
...
document.querySelector('.out-9').textContent = a9.toString().toLowerCase().includes(element.toString().toLowerCase())
}
