...
// TASK 02
// Выполним эмуляцию метода isArray. Т.е. функция берет переменную a2 и проверяет, что она является массивом. Возвращает true или false. 
// Один из возможных вариантов решения - применить try catch (вспоминаем. курс JS 2.0) и делаем push в переменную. 
...
// TASK 03
//По нажатию b-3 выполняется функция f3. Функция применяет к массиву a3 метод reduce и выводит переменные accum, item в консоль.
...
// братите внимание на область применения reduce - везде где нужно в результате получить одно число на основе массива - reduce подойдет. 
...
const f11 = () => {
    let res = a11.reduce((accum, item) => accum < item ? item : accum);
...
const f13 = () => {
    let res = a13.reduce((accum, item) => accum.length < item.length ? item.length : accum);
...
const f14 = () => {
    let res = a14.reduce((accum, item) => accum.length < item.length ? item : accum);
...
const f15 = () => {
    ...
    document.querySelector('.out-15').innerHTML = res.toFixed(3);
}
